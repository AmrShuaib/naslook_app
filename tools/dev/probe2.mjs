...
const browser = await chromium.launch({ headless: true, executablePath: '/opt/pw-browsers/chromium-1194/chrome-linux/chrome', args: ['--no-sandbox', '--use-fake-device-for-media-stream', '--use-fake-ui-for-media-stream', '--autoplay-policy=no-user-gesture-required'] });
const ctx = await browser.newContext({ viewport: { width: Number(process.env.VIEWPORT_W || 420), height: Number(process.env.VIEWPORT_H || 860) }, locale: 'ar', permissions: ['microphone', 'notifications', ...(process.env.GEO ? ['geolocation'] : [])], ...(process.env.GEO ? { geolocation: { latitude: Number(process.env.GEO.split(',')[0]), longitude: Number(process.env.GEO.split(',')[1]), accuracy: 30 } } : {}) });
if (!process.env.GUEST) await ctx.addInitScript((v) => { try { localStorage.setItem('flutter.naslife.session.v2', v); } catch (e) {} }, JSON.stringify(JSON.stringify({ token: 'tok', user: { id: 'SA0000001', nickname: 'amr' } })));
